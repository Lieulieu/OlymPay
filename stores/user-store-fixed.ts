...
type UserState = {
  ...
  createMultiChainWallet: () => Promise<void>;
    ...
    (set, get) => ({
      ...
      createMultiChainWallet: async () => {
        try {
          ...
          const res = await fetch("/api/wallet/create-multichain", {
            method: "POST",
            headers: {
              Authorization: `Bearer ${idToken}`,
              "Content-Type": "application/json",
          ...
          const json = await safeJson(res);
          if (!res.ok || !json?.solanaAddress || !json?.ethereumAddress) {
            throw new Error(json?.error || "Failed to create multi-chain wallet");
          }
          ...
          console.error(e);
          toast.error(e?.message || "Failed to create multi-chain wallet.");
          throw e;
        } finally {
          set({ walletLoading: false });
        }
